...
def transEDF2MAT(path, output_dir):
    ...
    for l in range(len(label_name)): 
        ...
        stages_lookup1 = {'Sleep stage 1': 1, 'Sleep stage 2': 2, 'Sleep stage 3': 3, 'Sleep stage 4': 3, 'Sleep stage ?': -1,
        ...
        time_end = annotations[0][stage_idx1[-1]] + annotations[1][stage_idx1[-1]]
        num1=[]
        k=0
        ...
        eeg_signal1 = raw1.readSignal(int(eeg0), int(time_begin)*fs_eeg, len(stages1)*epoch*fs_eeg) 
        eeg_signal2 = raw1.readSignal(int(eeg1), int(time_begin)*fs_eeg, len(stages1)*epoch*fs_eeg)
        print('-------------------------------\n')
        ...
        eog_signal1 = raw1.readSignal(int(eog), int(time_begin)*fs_eog, len(stages1)*epoch*fs_eog)
        print('-------------------------------\n')
        ...
        if emg_channel1 in labels1:
            ...
            emg_signal = raw1.readSignal(emg, int(time_begin)*fs_emg, len(stages1)*epoch*fs_emg)
        else:
            print('No EMG ')
        ...
        #滤波
        b1, a1 = signal.butter(2,[0.3*2/fs_eeg, 35*2/fs_eeg],'bandpass')
        eeg_signal1 = signal.filtfilt(b1, a1, eeg_signal1)
        eeg_signal2 = signal.filtfilt(b1, a1, eeg_signal2)
        
        b2, a2 = signal.butter(2,[0.3*2/fs_eog, 35 *2/fs_eog],'bandpass')
        ...
        #降采样
        eeg_signal1 = librosa.resample(y=np.squeeze(eeg_signal1), orig_sr=fs_eeg, target_sr=fs)
        eeg_signal2 = librosa.resample(y=np.squeeze(eeg_signal2), orig_sr=fs_eeg, target_sr=fs)
        eog_signal1 = librosa.resample(y=np.squeeze(eog_signal1), orig_sr=fs_eog, target_sr=fs)
        ...
        sio.savemat(os.path.join(output_dir, filename_seq),
                    {
                        "eeg1fpz": x1,
                        "eeg2pz": x2,
                        "eog": x3,
